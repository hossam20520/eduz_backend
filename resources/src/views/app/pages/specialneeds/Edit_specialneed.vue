<template>
  <div class="main-content">
    <breadcumb :page="'Update Specialneed'" :folder="$t('Specialneeds')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="Edit_Specialneed" v-if="!isLoading">
                  ...
                  <validation-provider name="category" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Choose_instation')">
                      <v-select
                        ...
                        v-model="specialneed.institution_id"
                  ...
                  <validation-provider
                    ...
                    <b-form-group :label="$t('ar_address')">
                      <b-form-input
                        ...
                        v-model="specialneed.ar_address"
                      ></b-form-input>
                      <b-form-invalid-feedback id="Name-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                  ...
                  <validation-provider
                    ...
                    <b-form-group :label="$t('en_address')">
                      <b-form-input
                        ...
                        v-model="specialneed.en_address"
                      ></b-form-input>
                      <b-form-invalid-feedback id="Name-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
...
.catch(response => {
 ...
 async uploadFiles() {
   ...
   try {
    ...
    Submit_Specialneed() {
      this.$refs.Edit_Specialneed.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    GetElements() {
      ...
      axios
        .get(`Specialneeds/${id}/edit`)
        .then(response => {
         
          this.specialneed = response.data.specialneed;
          this.areas = response.data.areas;
          this.govs = response.data.govs;
          console.log( response.data.govs);
          // this.files_activetiy = response.data.specialneed.activites_fiels.split(",").map(item => item.trim());
          this.specialneed.actives =    response.data.specialneed.actives 
          // this.files_activetiy = response.data.specialneed.activites_fiels.split(",").map(item => item.trim());
        ...
        this.$set(this.selectedOptions, section.id, section.drop.map(item => item.id));
          ...
          this.images = response.data.specialneed.images;
          this.images_tow = response.data.specialneed.images_tow;
          // this.image_tow = response.data.specialneed.image_tow;
          this.isLoading = false;
        })
        .catch(response => {
          setTimeout(() => {
    ...
    Update_Specialneed() {
      ...
      axios.post("Specialneeds/" + this.specialneed.id, self.data).then(response => {
          NProgress.done();
