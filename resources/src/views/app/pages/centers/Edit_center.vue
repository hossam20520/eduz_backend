<template>
  <div class="main-content">
    <breadcumb :page="'Update Center'" :folder="$t('Centers')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="Edit_Center" v-if="!isLoading">
                ...
                <b-row class="form-group">
                  <b-col md="12 mb-5">
                    <div
                      ...
                      dragText="Drag & Drop Multiple images For center"
...
.catch(response => {
 ...
 async uploadFiles() {
   ...
   try {
    ...
    Submit_Center() {
      this.$refs.Edit_Center.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    GetElements() {
      ...
      axios
        ...
        .then(response => {
          ...
          // this.files_activetiy = response.data.center.activites_fiels.split(",").map(item => item.trim());
          this.center.actives =    response.data.center.actives 
          // this.files_activetiy = response.data.center.activites_fiels.split(",").map(item => item.trim());
        ...
        this.$set(this.selectedOptions, section.id, section.drop.map(item => item.id));
          ...
          this.images = response.data.center.images;
          this.images_tow = response.data.center.images_tow;
          // this.image_tow = response.data.center.image_tow;
          this.isLoading = false;
        })
        .catch(response => {
          setTimeout(() => {
    ...
    Update_Center() {
      ...
      axios.post("Centers/" + this.center.id, self.data).then(response => {
          NProgress.done();
