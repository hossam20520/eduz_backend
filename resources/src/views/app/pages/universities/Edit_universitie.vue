<template>
  <div class="main-content">
    <breadcumb :page="'Update Universitie'" :folder="$t('Universities')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="Edit_Universitie" v-if="!isLoading">
                  ...
                  <validation-provider name="category" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Choose_instation')">
                      <v-select
                        ...
                        v-model="universitie.institution_id"
                  ...
                  <validation-provider
                    ...
                    <b-form-group :label="$t('ar_address')">
                      <b-form-input
                        ...
                        v-model="universitie.ar_address"
                      ></b-form-input>
                      <b-form-invalid-feedback id="Name-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                  ...
                  <validation-provider
                    ...
                    <b-form-group :label="$t('en_address')">
                      <b-form-input
                        ...
                        v-model="universitie.en_address"
                      ></b-form-input>
                      <b-form-invalid-feedback id="Name-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                ...
                <b-row class="form-group">
                  <b-col md="12 mb-5">
                    <div
                      ...
                      dragText="Drag & Drop Multiple images For universitie"
...
.catch(response => {
 ...
 async uploadFiles() {
   ...
   try {
    ...
    Submit_Universitie() {
      this.$refs.Edit_Universitie.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    GetElements() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.universitie = response.data.universitie;
          this.areas = response.data.areas;
          this.govs = response.data.govs;
          console.log( response.data.govs);
          // this.files_activetiy = response.data.universitie.activites_fiels.split(",").map(item => item.trim());
          this.universitie.actives =    response.data.universitie.actives 
          // this.files_activetiy = response.data.universitie.activites_fiels.split(",").map(item => item.trim());
        ...
        this.$set(this.selectedOptions, section.id, section.drop.map(item => item.id));
          ...
          this.images = response.data.universitie.images;
          this.images_tow = response.data.universitie.images_tow;
          // this.image_tow = response.data.universitie.image_tow;
          this.isLoading = false;
        })
        .catch(response => {
          setTimeout(() => {
    ...
    Update_Universitie() {
      ...
      axios.post("Universities/" + this.universitie.id, self.data).then(response => {
          NProgress.done();
