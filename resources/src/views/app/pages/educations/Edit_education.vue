<template>
  <div class="main-content">
    <breadcumb :page="'Update Education'" :folder="$t('Educations')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="Edit_Education" v-if="!isLoading">
                  ...
                  <validation-provider name="category" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Choose_instation')">
                      <v-select
                        ...
                        v-model="education.institution_id"
...
export default {
  ...
  methods: {
    ...
    Submit_Education() {
      this.$refs.Edit_Education.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
    ...
    Update_Education() {
      ...
      axios.post("Educations/" + this.education.id, self.data).then(response => {
          NProgress.done();
