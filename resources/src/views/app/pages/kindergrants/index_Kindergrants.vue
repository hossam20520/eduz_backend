<template>
  <div class="main-content">
    <breadcumb :page="$t('KindergrantsList')" :folder="$t('Kindergrants')"/>
    ...
    <div v-else>
      ...
      >
        ...
        <div slot="table-actions" class="mt-2 mb-3">
          ...
          <b-button @click="Kindergrant_PDF()" size="sm" variant="outline-success m-1">
            <i class="i-File-Copy"></i> PDF
          </b-button>
          <b-button @click="Kindergrant_Excel()" size="sm" variant="outline-danger m-1">
            <i class="i-File-Excel"></i> EXCEL
          </b-button>
          <b-button
            @click="Show_import_Kindergrants()"
            size="sm"
            variant="info m-1"
            v-if="currentUserPermissions && currentUserPermissions.includes('kindergrant_import')">
            <i class="i-Download"></i>
           {{  $t("import_Kindergrants")  }}
          </b-button>
          <router-link
            class="btn-sm btn btn-primary btn-icon m-1"
            v-if="currentUserPermissions && currentUserPermissions.includes('Kindergrants_add')"
            to="/app/Kindergrants/store">
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <router-link
              ...
              v-b-tooltip.hover
              title="View"
              :to="{ name:'detail_kindergrant', params: { id: props.row.id} }"
            >
            ...
            <router-link
              ...
              v-b-tooltip.hover
              title="Edit"
              :to="{ name:'edit_kindergrant', params: { id: props.row.id } }">
              <i class="i-Edit text-25 text-success"></i>
            </router-link>

            <a
              
              @click="Remove_Kindergrant(props.row.id)"
              v-b-tooltip.hover
              title="Delete" >
      ...
      <b-sidebar id="sidebar-right" :title="$t('Filter')" bg-variant="white" right shadow>
        <div class="px-3 py-2">
          <b-row>
            ...
            <b-col md="6" sm="12">
              <b-button
                @click="Get_Kindergrants(serverParams.page)"
                variant="primary m-1"
                size="sm"
                block
              >
      ...
      <!-- Modal Show Import Kindergrants -->
      <b-modal
        ok-only
        ok-title="Cancel"
        size="md"
        id="importKindergrants"
        :title="$t('import_Kindergrants')">
...
export default {
  ...
  methods: {
    ...
    Submit_import() {
      ...
      axios
        .post("Kindergrants/import/csv", self.data)
        .then(response => {
          ...
          axios
            ...
            .then(() => {
              this.$swal(
                ...
                this.$t("Delete.KindergrantDeleted"),
                "success"
              );
  ...
  created: function() {
    ...
    Fire.$on("Event_import", () => {
      setTimeout(() => {
        this.Get_Kindergrants(this.serverParams.page);
        this.$bvModal.hide("importKindergrants");
      }, 500);
    });
  }
};
